...
savingsSim.func <- function(df.in, modError.in) {
  ...
  ######  FORECAST MODEL  ######
    ...
    FC.mod.preds  <- as.matrix(X.post[,1:6]) %*% as.matrix(FC.mod.b.hat)
    ...
    XX.inv       <- solve(t(data.matrix(X.pre[,1:5])) %*% data.matrix(X.pre[,1:5]))
    post.mats    <- data.matrix(X.post[,1:5]) %*% XX.inv %*% t(data.matrix(X.post[,1:5]))
...
######  FULLY SPECIFIED PRE-POST MODEL  ######
    ...
    FPP.mod.sav   <- -1*(FPP.mod$coeff[8]*sum(X.post$prog_ind) + 
                           ...
                           FPP.mod$coeff[10]*sum(X.post$prog_x_noProd) +
                           FPP.mod$coeff[11]*sum(X.post$prog_x_prod1_x_HDD55)
                        )
    ...
    FPP.mod.seSav  <- sqrt(FPP.cov.out[8,8]*sum(X.post$prog_ind)^2 +
                            ...
                            FPP.cov.out[10,10]*sum(X.post$prog_x_noProd)^2 +
                            FPP.cov.out[11,11]*sum(X.post$prog_x_prod1_x_HDD55)^2 +

                            2*FPP.cov.out[8,9]*sum(X.post$prog_ind)*sum(X.post$prog_x_prod1) +
                            2*FPP.cov.out[8,10]*sum(X.post$prog_ind)*sum(X.post$prog_x_noProd) +
                            2*FPP.cov.out[8,11]*sum(X.post$prog_ind)*sum(X.post$prog_x_prod1_x_HDD55) +

                            2*FPP.cov.out[9,10]*sum(X.post$prog_x_prod1)*sum(X.post$prog_x_noProd) +
                            2*FPP.cov.out[9,11]*sum(X.post$prog_x_prod1)*sum(X.post$prog_x_prod1_x_HDD55) +

                            2*FPP.cov.out[10,11]*sum(X.post$prog_x_noProd)*sum(X.post$prog_x_prod1_x_HDD55)
                          )
