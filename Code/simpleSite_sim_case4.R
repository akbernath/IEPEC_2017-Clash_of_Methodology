...
names(X.full) <- c(simCoeff$Coefficient, "CDD65")
...
######  TRUE MODEL  ######
    ...
    kWh.meas     <- (as.matrix(df.in[,-9]) %*% simCoeff$Value)            + mod.epsilon
...
######  FORECAST MODEL  ######
    ...
    FC.mod.preds  <- as.matrix(X.post[,c(1:4,9)]) %*% as.matrix(FC.mod.b.hat)
    ...
    XX.inv      <- solve(t(data.matrix(X.pre[,c(1:4,9)])) %*% data.matrix(X.pre[,c(1:4,9)]))
    post.mats   <- data.matrix(X.post[,c(1:4,9)]) %*% XX.inv %*% t(data.matrix(X.post[,c(1:4,9)]))
...
######  FULLY SPECIFIED PRE-POST MODEL  ######
    ...
    FPP.mod.sav   <- -1*(FPP.mod$coeff[7]*sum(X.post$prog_ind) + 
                           ...
                           FPP.mod$coeff[9]*sum(X.post$prog_x_noProd)
                        )
    ...
    FPP.mod.seSav  <- sqrt(FPP.cov.out[7,7]*sum(X.post$prog_ind)^2 +
                            ...
                            FPP.cov.out[9,9]*sum(X.post$prog_x_noProd)^2 +

                            2*FPP.cov.out[7,8]*sum(X.post$prog_ind)*sum(X.post$prog_x_prod1) +
                            2*FPP.cov.out[7,9]*sum(X.post$prog_ind)*sum(X.post$prog_x_noProd) +

                            2*FPP.cov.out[8,9]*sum(X.post$prog_x_prod1)*sum(X.post$prog_x_noProd)
                           )
...
write.xlsx(simSummary, file.path(projPath,"Output","simData_simple - Case 4.xlsx"), 
           sheetName="Sim Out", col.names=T, row.names=F, append=F)
write.xlsx(overspec.sim, file.path(projPath,"Output","simData_simple - Case 4.xlsx"), 
