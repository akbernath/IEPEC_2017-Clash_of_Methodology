...
######  FORECAST MODEL  ######
    ...
    FC.mod.preds  <- as.matrix(X.post[,1:3]) %*% as.matrix(FC.mod.b.hat)
...
write.xlsx(simSummary, file.path(projPath,"Output","simData_simple - Case 3.xlsx"), 
           sheetName="Sim Out", col.names=T, row.names=F, append=F)
write.xlsx(overspec.sim, file.path(projPath,"Output","simData_simple - Case 3.xlsx"), 
