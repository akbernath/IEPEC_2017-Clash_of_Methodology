...
savingsSim.func <- function(df.in, modError.in) {
  ...
  ######  FORECAST MODEL  ######
    ...
    FC.mod.preds  <- as.matrix(X.post[,1:5]) %*% as.matrix(FC.mod.b.hat)
...
######  FULLY SPECIFIED PRE-POST MODEL  ######
    ...
    ##  Estimate savings & SE(savings)
    FPP.mod.sav   <- -1*(FPP.mod$coeff[7]*sum(X.post$prog_ind) + 
                           FPP.mod$coeff[8]*sum(X.post$prog_x_prod1) +
                           FPP.mod$coeff[9]*sum(X.post$prog_x_prod2) +
    ...
    FPP.mod.seSav  <- sqrt(FPP.cov.out[7,7]*sum(X.post$prog_ind)^2 +
                            FPP.cov.out[8,8]*sum(X.post$prog_x_prod1)^2 +
                            FPP.cov.out[9,9]*sum(X.post$prog_x_prod2)^2 +
                            FPP.cov.out[10,10]*sum(X.post$prog_x_noProd)^2 +

                            2*FPP.cov.out[7,8]*sum(X.post$prog_ind)*sum(X.post$prog_x_prod1) +
                            2*FPP.cov.out[7,9]*sum(X.post$prog_ind)*sum(X.post$prog_x_prod2) +
                            2*FPP.cov.out[7,10]*sum(X.post$prog_ind)*sum(X.post$prog_x_noProd) +

                            2*FPP.cov.out[8,9]*sum(X.post$prog_x_prod1)*sum(X.post$prog_x_prod2) +
                            2*FPP.cov.out[8,10]*sum(X.post$prog_x_prod1)*sum(X.post$prog_x_noProd) +

                            2*FPP.cov.out[9,10]*sum(X.post$prog_x_prod2)*sum(X.post$prog_x_noProd)
                          )
