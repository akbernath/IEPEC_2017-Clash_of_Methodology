...
######  FULLY SPECIFIED PRE-POST MODEL  ######
    ...
    ##  Estimate savings & SE(savings)
    FPP.mod.sav   <- -1*(FPP.mod$coeff[6]*sum(X.post$prog_ind) + 
                           FPP.mod$coeff[7]*sum(X.post$prog_x_prod1) +
                           FPP.mod$coeff[8]*sum(X.post$prog_x_noProd)
                        )
    FPP.mod.bias   <- true.sav - FPP.mod.sav
    FPP.mod.pctErr <- FPP.mod.bias/true.sav
    FPP.cov.out    <- vcov(FPP.mod)
    FPP.mod.seSav  <- sqrt(FPP.cov.out[6,6]*sum(X.post$prog_ind)^2 +
                            FPP.cov.out[7,7]*sum(X.post$prog_x_prod1)^2 +
                            FPP.cov.out[8,8]*sum(X.post$prog_x_noProd)^2 +

                            2*FPP.cov.out[6,7]*sum(X.post$prog_ind)*sum(X.post$prog_x_prod1) +
                            2*FPP.cov.out[6,8]*sum(X.post$prog_ind)*sum(X.post$prog_x_noProd) +

                            2*FPP.cov.out[7,8]*sum(X.post$prog_x_prod1)*sum(X.post$prog_x_noProd)
                           )
...
write.xlsx(simSummary, file.path(projPath,"Output","simData_simple - Case 5.xlsx"), 
           sheetName="Sim Out", col.names=T, row.names=F, append=F)
write.xlsx(overspec.sim, file.path(projPath,"Output","simData_simple - Case 5.xlsx"), 
